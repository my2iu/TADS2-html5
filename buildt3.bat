...
set CCOPTS=-s ASSERTIONS=1 -s ASYNCIFY -s WASM=1 -s DISABLE_EXCEPTION_CATCHING=0 -DHTML5 -DOSANSI -DUSE_HTML -DUNIX -DHAVE_STRCASECMP  -DVMGLOB_VARS -DOS_DECLARATIVE_TLS -DTC_TARGET_T3 -DUSE_GENRAND -DUSE_DOSEXT -O2 
set LDOPTS=-s ASSERTIONS=1 -s ASYNCIFY -s "ASYNCIFY_IMPORTS=['async_printz']" -s WASM=1 -s MODULARIZE=1 -s DISABLE_EXCEPTION_CATCHING=0 -s "EXPORT_NAME='TadsLoader'" -O2

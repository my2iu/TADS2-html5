...
// The old vmerr.h actually works in WebAssembly, but it's easier to 
// debug things with realy exceptions
...
struct FakeExceptionForFinally 
{
};

// Trick found on StackOverflow to emulate a finally in a manner that matches
// the existing t3 emulated exception pattern
template <typename CodeLambda, typename FinallyLambda>
void err_try_with_finally(CodeLambda codeLambda, FinallyLambda finallyLambda) 
{
	try	{
		codeLambda();
	}
...
template <typename CodeLambda>
void err_try_with_finally(CodeLambda codeLambda) 
{
	codeLambda();
}
...
#define err_try err_try_with_finally([&]() { try

#define err_catch(exc) catch(CVmException *exc)

#define err_catch_disc catch(...)


#define err_finally_after_catch_disc }, [&](){  
#define err_finally catch(FakeExceptionForFinally e) {}}, [&](){  
